...
var
  fmManImpCtaNovo: TfmManImpCtaNovo;
...
function TfmManImpCtaNovo.ShowModal(NumCta: integer; DteCta: TDateTime): integer;
begin
  //inherited Show;
  //
  PedCt2.Active := False;
...
function TfmManImpCtaNovo.EnviaEmail(NumCta: integer): Boolean;
var
  strPDF: string;
...
begin
  ...
  begin
    ...
    begin
      try
        //fmManPri.enviaEmail(true,PAnsiChar('Cotação ' + inttostr(numcta)),PAnsiChar('Segue em anexo cotação ' + inttostr(numcta)),
        //                    PAnsiChar(strEmailCli), VarArrayOf([strPDF]));
...
function TfmManImpCtaNovo.SavePDF(Path: string): boolean;
var
...
procedure TfmManImpCtaNovo.qSubDetalheBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TfmManImpCtaNovo.QuickRep1Preview(Sender: TObject);
begin
...
procedure TfmManImpCtaNovo.PedCt2CalcFields(DataSet: TDataSet);
begin
  if PedCt2QTPRE2.AsFloat > 0 then
    PedCt2VALLIQTRB.AsFloat := fRound(PedCt2TOTITETRB.AsFloat / PedCt2QTPRE2.AsFloat, 2);
end;

procedure TfmManImpCtaNovo.FormCreate(Sender: TObject);
begin
...
procedure TfmManImpCtaNovo.QRDBText9Print(sender: TObject;
  var Value: string);
begin
  Value := PedCtaCODCLI.AsString + ' - ' + Trim(Value);
end;

procedure TfmManImpCtaNovo.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TfmManImpCtaNovo.QRDBText50Print(sender: TObject;
  var Value: string);
begin
  Value := 'CNPJ: ' + Value;
end;

procedure TfmManImpCtaNovo.QRDBText28Print(sender: TObject;
  var Value: string);
begin
  Value := 'I.E.: ' + Value;
end;

procedure TfmManImpCtaNovo.PedCtaAfterOpen(DataSet: TDataSet);
begin
...
procedure TfmManImpCtaNovo.QRDBText51Print(sender: TObject;
  var Value: string);
begin
  Value := fmManGdb.BuscaSimples('', 'FRT', '', 'select (CAST(ID as CHAR(1)) ||'' - ''|| DESCFRT) FRT FROM TIPFRT WHERE ID = ' + Value);
end;

procedure TfmManImpCtaNovo.QRLabel37Print(sender: TObject;
  var Value: String);
begin
if (pedct2.FieldByName('qtdatendida').AsInteger > 0) then
  begin
    QRLabel17.Caption := 'Quantidade';
    QRLabel37.Caption := pedCt2.FieldByName('QTPRE2').AsString;
    lblFaltas.Caption := '';
  ...
  begin
    ...
    QRLabel37.Caption := pedCt2.FieldByName('qtdatendida').AsString;
    lblFaltas.Caption := '   FALTAS NO PEDIDO';
    //lbQtd.Caption := 'Qtd. Falta';
  end;
end;

procedure TfmManImpCtaNovo.QRLabel17Print(sender: TObject;
  var Value: String);
begin
  if (pedct2.FieldByName('qtdatendida').AsInteger >= 0) then
  begin
...
procedure TfmManImpCtaNovo.bandSumarioBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PEDCT3.Active := False;
  PEDCT3.Active := True;
...
procedure TfmManImpCtaNovo.QRDBText33Print(sender: TObject;
  var Value: String);
var
   strAux: String;
begin
