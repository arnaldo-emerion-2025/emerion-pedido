inherited fmPedR15: TfmPedR15
  Left = 475
  Top = 98
  Caption = 'Analise de Pedidos Realizados no Periodo'
  ClientHeight = 482
  ClientWidth = 786
  Position = poDesigned
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object Label17: TLabel
    Left = 4
    Top = 132
    Width = 118
    Height = 13
    Caption = 'Faturados no Periodo de'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label18: TLabel
    Left = 258
    Top = 132
    Width = 6
    Height = 13
    Caption = 'a'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object BbPsqEmp: TSpeedButton
    Left = 250
    Top = 14
    Width = 23
    Height = 21
    Hint = 'Pesquisar Empresas Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = BbPsqEmpClick
  end
  object Label10: TLabel
    Left = 4
    Top = 0
    Width = 154
    Height = 13
    Caption = 'Informações para Pesquisa'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Bevel1: TBevel
    Left = 164
    Top = 6
    Width = 620
    Height = 3
    Shape = bsTopLine
    Style = bsRaised
  end
  object Label2: TLabel
    Left = 3
    Top = 165
    Width = 121
    Height = 13
    Caption = 'Pedidos Selecionados'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Bevel2: TBevel
    Left = 163
    Top = 172
    Width = 620
    Height = 3
    Shape = bsTopLine
    Style = bsRaised
  end
  object Label5: TLabel
    Left = 151
    Top = 17
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label9: TLabel
    Left = 151
    Top = 132
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label26: TLabel
    Left = 4
    Top = 16
    Width = 41
    Height = 13
    Caption = 'Empresa'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label22: TLabel
    Left = 4
    Top = 109
    Width = 46
    Height = 13
    Caption = 'Vendedor'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label3: TLabel
    Left = 4
    Top = 63
    Width = 104
    Height = 13
    Caption = 'Grupo de Vendedores'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label20: TLabel
    Left = 4
    Top = 86
    Width = 122
    Height = 13
    Caption = 'Categoria de Vendedores'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label21: TLabel
    Left = 151
    Top = 86
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label19: TLabel
    Left = 151
    Top = 63
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label8: TLabel
    Left = 151
    Top = 109
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object bPsqCCl: TSpeedButton
    Left = 250
    Top = 83
    Width = 23
    Height = 21
    Hint = 'Pesquisar Categorias de Vendedores Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqCClClick
  end
  object bPsqGve: TSpeedButton
    Left = 250
    Top = 60
    Width = 23
    Height = 21
    Hint = 'Pesquisar Grupos de Vendedores Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqGveClick
  end
  object bPsqVen: TSpeedButton
    Left = 250
    Top = 106
    Width = 23
    Height = 21
    Hint = 'Pesquisar Vendedores Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqVenClick
  end
  object Label7: TLabel
    Left = 4
    Top = 39
    Width = 33
    Height = 13
    Caption = 'Cliente'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label11: TLabel
    Left = 151
    Top = 40
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object bPsqCli: TSpeedButton
    Left = 250
    Top = 37
    Width = 23
    Height = 21
    Hint = 'Pesquisar Clientes Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqCliClick
  end
  object EdPsqDteres1: TdxColorDateEdit
    Left = 162
    Top = 128
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 10
    Date = -700000
    SaveTime = False
    UseEditMask = True
    CorDeFoco = clInfoBk
    StoredValues = 4
  end
  object EdPsqDteres2: TdxColorDateEdit
    Left = 273
    Top = 128
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 11
    Date = -700000
    SaveTime = False
    UseEditMask = True
    CorDeFoco = clInfoBk
    StoredValues = 4
  end
  object EdPsqCodEmp: TdxColorEdit
    Left = 162
    Top = 13
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 0
    OnExit = EdPsqCodEmpExit
    OnKeyPress = EdPsqCodEmpKeyPress
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdPsqApeEmp: TdxColorEdit
    Left = 273
    Top = 13
    Width = 512
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 1
    CharCase = ecUpperCase
    CorDeFoco = clBlack
  end
  object bSelecionar: TBitBtn
    Left = 454
    Top = 130
    Width = 164
    Height = 39
    Caption = '&Selecionar'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 12
    OnClick = bSelecionarClick
    Glyph.Data = {
      
      3232323232323232323232323232323232323232323232323232}
  end
  object grR15: TdxDBGrid
    Left = 2
    Top = 180
    Width = 783
    Height = 302
    Bands = <
      item
        Width = 2109
      end>
    DefaultLayout = False
    HeaderPanelRowCount = 1
    KeyField = 'NUMRES'
    ShowGroupPanel = True
    ShowSummaryFooter = True
    SummaryGroups = <
      item
        DefaultGroup = True
        SummaryItems = <
          item
            ColumnName = 'TOTBRT'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            ColumnName = 'TOTPER'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            ColumnName = 'CSTPRT'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            ColumnName = 'TOTLIQ'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            ColumnName = 'CSTPFI'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            ColumnName = 'CSTVME'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            ColumnName = 'DSCPER'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            ColumnName = 'DSCPRT'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            ColumnName = 'LUCROP'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            ColumnName = 'LUCROL'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            ColumnName = 'TOTNAT'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end>
        Name = 'grR15SummaryGroup1'
      end>
    SummarySeparator = ', '
    BorderStyle = bsNone
    Color = clInfoBk
    Font.Charset = ANSI_CHARSET
    Font.Color = clNavy
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    PopupMenu = PopUpMenu
    TabOrder = 14
    BandColor = clMenu
    BandFont.Charset = ANSI_CHARSET
    BandFont.Color = clWindowText
    BandFont.Height = -11
    BandFont.Name = 'Tahoma'
    BandFont.Style = [fsBold]
    DataSource = DsR15
    Filter.Active = True
    Filter.AutoDataSetFilter = True
    Filter.Criteria = {00000000}
    GroupPanelColor = clAppWorkSpace
    GroupPanelFontColor = clInfoBk
    GroupNodeColor = clInfoBk
    HeaderFont.Charset = ANSI_CHARSET
    HeaderFont.Color = clWindowText
    HeaderFont.Height = -11
    HeaderFont.Name = 'Tahoma'
    HeaderFont.Style = [fsBold]
    LookAndFeel = lfFlat
    OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEnterShowEditor, edgoImmediateEditor, edgoTabThrough, edgoVertThrough]
    OptionsCustomize = [edgoBandPanelSizing, edgoBandSizing, edgoColumnMoving, edgoColumnSizing, edgoFullSizing]
    OptionsDB = [edgoCanNavigation, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoPreview, edgoRowSelect, edgoUseBitmap]
    PreviewFont.Charset = ANSI_CHARSET
    PreviewFont.Color = clBlue
    PreviewFont.Height = -11
    PreviewFont.Name = 'Tahoma'
    PreviewFont.Style = []
    PreviewLines = 0
    RowFooterColor = clInfoBk
    RowFooterTextColor = clRed
    ShowGrid = False
    ShowHiddenInCustomizeBox = True
    ShowRowFooter = True
    OnReloadGroupList = grR15ReloadGroupList
    object NUMRES: TdxDBGridMaskColumn
      Alignment = taLeftJustify
      Caption = 'Pedido'
      Width = 74
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NUMRES'
      SummaryFooterType = cstCount
      SummaryFooterFormat = '####0'
    end
    object DTERES: TdxDBGridDateColumn
      Caption = 'Emitido Em'
      Width = 93
      BandIndex = 0
      RowIndex = 0
      FieldName = 'DTERES'
    end
    object VENDEDOR: TdxDBGridMaskColumn
      Caption = 'Vendedor'
      Width = 123
      BandIndex = 0
      RowIndex = 0
      FieldName = 'VENDEDOR'
    end
    object CLIENTE: TdxDBGridMaskColumn
      Caption = 'Cliente'
      Width = 180
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CLIENTE'
    end
    object DTEFAT: TdxDBGridDateColumn
      Caption = 'Dt Fat.'
      Width = 75
      BandIndex = 0
      RowIndex = 0
      FieldName = 'DTEFAT'
    end
    object NRONFS: TdxDBGridMaskColumn
      Alignment = taLeftJustify
      Caption = 'NF'
      Width = 71
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NRONFS'
    end
    object TOTGE2: TdxDBGridMaskColumn
      Caption = 'Ped. Faturado'
      HeaderAlignment = taRightJustify
      Width = 88
      BandIndex = 0
      RowIndex = 0
      FieldName = 'TOTGE2'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
      EditMask = '0.00'
    end
    object TOTIPI: TdxDBGridMaskColumn
      Caption = 'Total IPI'
      HeaderAlignment = taRightJustify
      Width = 79
      BandIndex = 0
      RowIndex = 0
      FieldName = 'TOTIPI'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
      EditMask = '0.00'
    end
    object TOTSUB: TdxDBGridMaskColumn
      Caption = 'Total ST'
      HeaderAlignment = taRightJustify
      Width = 76
      BandIndex = 0
      RowIndex = 0
      FieldName = 'TOTSUB'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
      EditMask = '0.00'
    end
    object TOTBRT: TdxDBGridMaskColumn
      Caption = 'Total Bruto'
      HeaderAlignment = taRightJustify
      Width = 91
      BandIndex = 0
      RowIndex = 0
      FieldName = 'TOTBRT'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
    end
    object TOTPER: TdxDBGridColumn
      Caption = 'Bruto - Desc'
      HeaderAlignment = taRightJustify
      Visible = False
      Width = 94
      BandIndex = 0
      RowIndex = 0
      FieldName = 'TOTPER'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
    end
    object TOTLIQ: TdxDBGridMaskColumn
      Caption = 'Total Liq'
      HeaderAlignment = taRightJustify
      Width = 76
      BandIndex = 0
      RowIndex = 0
      FieldName = 'TOTLIQ'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
    end
    object CSTPRT: TdxDBGridColumn
      HeaderAlignment = taRightJustify
      Width = 73
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CSTPRT'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
    end
    object CSTPFI: TdxDBGridColumn
      HeaderAlignment = taRightJustify
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CSTPFI'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
    end
    object CSTVME: TdxDBGridColumn
      HeaderAlignment = taRightJustify
      Width = 70
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CSTVME'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
    end
    object DSCPER: TdxDBGridMaskColumn
      Caption = 'Desc Per'
      HeaderAlignment = taRightJustify
      Width = 73
      BandIndex = 0
      RowIndex = 0
      FieldName = 'DSCPER'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
      EditMask = '0.00'
    end
    object DSCPRT: TdxDBGridMaskColumn
      Caption = 'Desc Prat'
      HeaderAlignment = taRightJustify
      Width = 73
      BandIndex = 0
      RowIndex = 0
      FieldName = 'DSCPRT'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
      EditMask = '0.00'
    end
    object DIFDSC: TdxDBGridMaskColumn
      Caption = '% Dif Desc'
      HeaderAlignment = taRightJustify
      Visible = False
      Width = 82
      BandIndex = 0
      RowIndex = 0
      FieldName = 'DIFDSC'
    end
    object MARPRE: TdxDBGridMaskColumn
      Caption = 'Mk - Brt'
      HeaderAlignment = taRightJustify
      Width = 84
      BandIndex = 0
      RowIndex = 0
      FieldName = 'MARPRE'
    end
    object MARPED: TdxDBGridMaskColumn
      Caption = 'Mk - P'
      HeaderAlignment = taRightJustify
      Width = 68
      BandIndex = 0
      RowIndex = 0
      FieldName = 'MARPED'
    end
    object MARGBRT: TdxDBGridMaskColumn
      Caption = 'Margem - Brt'
      HeaderAlignment = taRightJustify
      Visible = False
      Width = 104
      BandIndex = 0
      RowIndex = 0
      FieldName = 'MARGBRT'
      EditMask = '0.00'
    end
    object MARGPED: TdxDBGridMaskColumn
      Caption = 'Margem - P'
      HeaderAlignment = taRightJustify
      Width = 92
      BandIndex = 0
      RowIndex = 0
      FieldName = 'MARGPED'
      EditMask = '0.00'
    end
    object LUCROP: TdxDBGridMaskColumn
      Caption = 'Final Bruto'
      HeaderAlignment = taRightJustify
      Width = 69
      BandIndex = 0
      RowIndex = 0
      FieldName = 'LUCROP'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
    end
    object LUCROL: TdxDBGridMaskColumn
      Caption = 'Final Liq'
      HeaderAlignment = taRightJustify
      Width = 59
      BandIndex = 0
      RowIndex = 0
      FieldName = 'LUCROL'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
    end
    object TOTNAT: TdxDBGridMaskColumn
      Caption = 'Não Atendido'
      HeaderAlignment = taRightJustify
      Width = 72
      BandIndex = 0
      RowIndex = 0
      FieldName = 'TOTNAT'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
    end
    object NOMCLI: TdxDBGridColumn
      Caption = 'Razão Social'
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NOMCLI'
    end
    object CGCCLI: TdxDBGridMaskColumn
      Caption = 'CNPJ'
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CGCCLI'
    end
  end
  object bImprimir: TBitBtn
    Left = 620
    Top = 130
    Width = 164
    Height = 39
    Caption = '&Imprimir'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 13
    OnClick = bImprimirClick
    Glyph.Data = {
      
      D000}
  end
  object EdPsqCodVen: TdxColorEdit
    Left = 162
    Top = 105
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 8
    OnExit = EdPsqCodVenExit
    OnKeyPress = EdPsqCodEmpKeyPress
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdPsqCodGve: TdxColorEdit
    Left = 162
    Top = 59
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 4
    OnExit = EdPsqCodGveExit
    OnKeyPress = EdPsqCodEmpKeyPress
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdPsqCodCve: TdxColorEdit
    Left = 162
    Top = 82
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 6
    OnExit = EdPsqCodCveExit
    OnKeyPress = EdPsqCodEmpKeyPress
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdPsqNomVen: TdxColorEdit
    Left = 273
    Top = 105
    Width = 512
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 9
    CharCase = ecUpperCase
    CorDeFoco = clBlack
  end
  object EdPsqNomGve: TdxColorEdit
    Left = 273
    Top = 59
    Width = 512
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 5
    CharCase = ecUpperCase
    CorDeFoco = clBlack
  end
  object EdPsqNomCve: TdxColorEdit
    Left = 273
    Top = 82
    Width = 512
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 7
    CharCase = ecUpperCase
    CorDeFoco = clBlack
  end
  object EdPsqCodCli: TdxColorEdit
    Left = 162
    Top = 36
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 2
    OnExit = EdPsqCodCliExit
    OnKeyPress = EdPsqCodEmpKeyPress
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdPsqNomCli: TdxColorEdit
    Left = 273
    Top = 36
    Width = 512
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 3
    CharCase = ecUpperCase
    CorDeFoco = clBlack
  end
  object PedR15: TwwQuery
    CachedUpdates = True
    DatabaseName = 'ISade'
    SQL.Strings = (
      ' Select '
      ' PedLb2.CodEmp,'
      ' PedLb2.DteRes,'
      ' PedLb2.NumRes,'
      ' PedLb2.SeqLib,'
      ' PedLib.NroNfs,'
      ' PedLib.DteFat,'
      ' PedLib.CodCli,'
      ' FinCli.ApeCli,'
      ' FinCli.NomCli,'
      ' FinCli.CgcCli,'
      ' PedLib.CodVen,'
      ' FinVen.ApeVen,'
      ' Round(Sum((PedLb2.UltQts * PedLb2.VlqLb2)),2) TotLiq,'
      ' Round(Sum((PedLb2.UltQts * PedLb2.VluLb2)) ,2) TotBrt,'
      ' Round(Sum(PedLb2.TotCst),4) CstPrt,'
      ' Round(Sum(PedLb2.UltQts * PedLb2.VpfCst),4) CstPfi,'
      ' Round(Sum(PedLb2.UltQts * PedLb2.VmeCst),4) CstVme,'
      
        ' Round(Sum(PedLb2.UltQts * (( PedLb2.VluLb2 * PedLb2.DscPer )/10' +
        '0)),2) as DscPer,'
      ' Round(Sum(PedLb2.TotDsc),2) DscPrt,'
      
        ' case when (Round(Sum(PedLb2.UltQts * (( PedLb2.VluLb2 * PedLb2.' +
        'DscPer )/100)),2) > 0) THEN ROUND(((Round(Sum(PedLb2.TotDsc),2) ' +
        '* 100)/Round(Sum(PedLb2.UltQts * (( PedLb2.VluLb2 * PedLb2.DscPe' +
        'r )/100)),2)) - 100,2) else 0 end DifDsc,'
      
        ' case when (Round(Sum(PedLb2.TotCst),4) > 0) THEN ROUND(((Round(' +
        'Sum((PedLb2.UltQts * PedLb2.VluLb2)),2) - Round(Sum(PedLb2.TotCs' +
        't),4)) * 100)/Round(Sum(PedLb2.TotCst),4),2) else 0 end MarPre,'
      
        ' case when (Round(Sum(PedLb2.TotCst),4) > 0) THEN ROUND(((Round(' +
        'Sum((PedLb2.UltQts * PedLb2.VlqLb2)),2) - Round(Sum(PedLb2.TotCs' +
        't),4)) * 100)/Round(Sum(PedLb2.TotCst),4),2) else 0 end MarPed,'
      
        ' ROUND(Round(Sum((PedLb2.UltQts * PedLb2.VlqLb2)),2) - Round(Sum' +
        '(PedLb2.TotCst),4),4) LuCroL,'
      
        ' ROUND(Round(Sum((PedLb2.UltQts * PedLb2.VluLb2)) ,2) - Round(Su' +
        'm(PedLb2.TotCst),4),4) LuCroP,'
      
        ' Round(Sum(PedLb2.SldLb2 * ( PedLb2.VlqLb2 - PedLb2.VdrLb2 )),4)' +
        ' as TotNat,'
      
        ' ROUND(Round(Sum((PedLb2.UltQts * PedLb2.VluLb2)) ,2) -Round(Sum' +
        '(PedLb2.UltQts * (( PedLb2.VluLb2 * PedLb2.DscPer )/100)),2),2) ' +
        'TotPer,'
      ' Round(Sum(PedLb2.totipi),2) totipi,'
      ' Round(Sum(PedLB2.totsub),2) totsub,'
      ' Round(Sum(PedLB2.totge2),2) totge2,'
      
        ' Round(((Sum(PedLb2.UltQts*PedLb2.VluLb2) - Sum(PedLb2.UltQts*Pe' +
        'dRe2.VCSRE2))/Sum(PedLb2.UltQts*PedLb2.VluLb2))*100,4) MargBrt,'
      
        ' Round(((Sum(PedLb2.UltQts*PedLb2.VlqLb2) - Sum(PedLb2.UltQts*Pe' +
        'dRe2.VCSRE2))/Sum(PedLb2.UltQts*PedLb2.VlqLb2))*100,4) MargPed,'
      ' IntStrZeros(PedLib.CodCli,5)||'#39'-'#39'||FinCli.ApeCli as Cliente,'
      ' IntStrZeros(PedLib.CodVen,3)||'#39'-'#39'||FinVen.ApeVen as Vendedor'
      ' From PedLb2'
      ' join PedLib on PedLb2.CodEmp = PedLib.CodEmp'
      ' and PedLb2.DteRes = PedLib.DteRes'
      ' and PedLb2.NumRes = PedLib.NumRes'
      ' and PedLb2.SeqLib = PedLib.SeqLib'
      ' join FinVen on PedLib.CodVen = FinVen.CodVen'
      ' join FinCli on PedLib.CodCli = FinCli.CodCli'
      ' join PedRe2 on PedRe2.CodEmp = PedLb2.CodEmp'
      ' and PedRe2.DteRes = PedLb2.DteRes'
      ' and PedRe2.NumRes = PedLb2.NumRes'
      ' and PedRe2.seqre2 = PedLb2.seqre2'
      ' Where 1 = 2'
      
        ' Group by PedLb2.CodEmp, PedLb2.DteRes,PedLb2.NumRes,PedLb2.SeqL' +
        'ib,PedLib.NroNfs,PedLib.DteFat,PedLib.CodCli,FinCli.ApeCli,FinCl' +
        'i.NomCli,FinCli.CgcCli,PedLib.CodVen,FinVen.ApeVen'
      
        ' Order by PedLb2.CodEmp, PedLb2.DteRes,PedLb2.NumRes,PedLb2.SeqL' +
        'ib,PedLib.NroNfs,PedLib.DteFat,PedLib.CodCli,FinCli.ApeCli,PedLi' +
        'b.CodVen,FinVen.ApeVen')
    ValidateWithMask = True
    Left = 216
    Top = 285
    object PedR15CODEMP: TIntegerField
      FieldName = 'CODEMP'
    end
    object PedR15DTERES: TDateTimeField
      FieldName = 'DTERES'
    end
    object PedR15NUMRES: TIntegerField
      FieldName = 'NUMRES'
    end
    object PedR15NRONFS: TIntegerField
      FieldName = 'NRONFS'
    end
    object PedR15DTEFAT: TDateTimeField
      FieldName = 'DTEFAT'
    end
    object PedR15TOTBRT: TFloatField
      FieldName = 'TOTBRT'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15TOTLIQ: TFloatField
      FieldName = 'TOTLIQ'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15DSCPER: TFloatField
      FieldName = 'DSCPER'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15DSCPRT: TFloatField
      FieldName = 'DSCPRT'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15DIFDSC: TFloatField
      FieldName = 'DIFDSC'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15MARPRE: TFloatField
      FieldName = 'MARPRE'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15MARPED: TFloatField
      FieldName = 'MARPED'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15LUCROL: TFloatField
      FieldName = 'LUCROL'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15LUCROP: TFloatField
      FieldName = 'LUCROP'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15TOTNAT: TFloatField
      FieldName = 'TOTNAT'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15CLIENTE: TStringField
      FieldName = 'CLIENTE'
      FixedChar = True
      Size = 100
    end
    object PedR15VENDEDOR: TStringField
      FieldName = 'VENDEDOR'
      FixedChar = True
      Size = 100
    end
    object PedR15CSTPRT: TFloatField
      DisplayLabel = 'Ct - PED'
      FieldName = 'CSTPRT'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15CSTPFI: TFloatField
      DisplayLabel = 'Ct - UC'
      FieldName = 'CSTPFI'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15CSTVME: TFloatField
      DisplayLabel = 'Ct - ME'
      FieldName = 'CSTVME'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15TOTPER: TFloatField
      DisplayLabel = 'Bruto + Desc'
      FieldName = 'TOTPER'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedR15TOTIPI: TFloatField
      FieldName = 'TOTIPI'
      DisplayFormat = '0.00'
      EditFormat = '0.00'
    end
    object PedR15TOTSUB: TFloatField
      FieldName = 'TOTSUB'
      DisplayFormat = '0.00'
      EditFormat = '0.00'
    end
    object PedR15TOTGE2: TFloatField
      FieldName = 'TOTGE2'
      DisplayFormat = '0.00'
      EditFormat = '0.00'
    end
    object PedR15SEQLIB: TIntegerField
      FieldName = 'SEQLIB'
    end
    object PedR15CODCLI: TIntegerField
      FieldName = 'CODCLI'
    end
    object PedR15APECLI: TStringField
      FieldName = 'APECLI'
    end
    object PedR15CODVEN: TIntegerField
      FieldName = 'CODVEN'
    end
    object PedR15APEVEN: TStringField
      FieldName = 'APEVEN'
    end
    object PedR15MARGBRT: TFloatField
      FieldName = 'MARGBRT'
      DisplayFormat = '0.00'
      EditFormat = '0.00'
    end
    object PedR15MARGPED: TFloatField
      FieldName = 'MARGPED'
      DisplayFormat = '0.00'
      EditFormat = '0.00'
    end
    object PedR15CGCCLI: TStringField
      FieldName = 'CGCCLI'
    end
    object PedR15NOMCLI: TStringField
      FieldName = 'NOMCLI'
      Size = 50
    end
  end
  object DsR15: TwwDataSource
    DataSet = PedR15
    Left = 244
    Top = 285
  end
  object quSql: TwwQuery
    DatabaseName = 'ISade'
    UniDirectional = True
    ValidateWithMask = True
    Left = 216
    Top = 313
  end
  object SaveDialog: TSaveDialog
    FileName = 'ExpGrid'
    Options = [ofOverwritePrompt, ofHideReadOnly]
    Left = 244
    Top = 313
  end
  object dxComponentPrinter: TdxComponentPrinter
    CurrentLink = dxComponentPrinterLink1
    DateFormat = 1
    PreviewOptions.Caption = 'Visualizando Relatorio'
    PreviewOptions.RegistryPath = 'Software\Developer Express\PrintingSystem\DesignTime'
    PreviewOptions.PreviewBoundsRect = {00000000000000000005000004030000}
    TimeFormat = 1
    Left = 244
    Top = 341
    object dxComponentPrinterLink1: TdxDBGridReportLink
      Active = True
      Component = grR15
      DateTime = 45594.6099322685
      DesignerHelpContext = 0
      PrinterPage.DMPaper = 9
      PrinterPage.Footer = 6350
      PrinterPage.Header = 6700
      PrinterPage.Margins.Bottom = 6400
      PrinterPage.Margins.Left = 6400
      PrinterPage.Margins.Right = 6400
      PrinterPage.Margins.Top = 45000
      PrinterPage.Orientation = poLandscape
      PrinterPage.PageFooter.Font.Charset = DEFAULT_CHARSET
      PrinterPage.PageFooter.Font.Color = clWindowText
      PrinterPage.PageFooter.Font.Height = -11
      PrinterPage.PageFooter.Font.Name = 'Tahoma'
      PrinterPage.PageFooter.Font.Style = []
      PrinterPage.PageFooter.RightTitle.Strings = (
        'Pagina No. [Page #]')
      PrinterPage.PageHeader.Background.Brush.Color = clBlack
      PrinterPage.PageHeader.Background.PictureMode = ppmTile
      PrinterPage.PageHeader.CenterTextAlignY = taTop
      PrinterPage.PageHeader.Font.Charset = DEFAULT_CHARSET
      PrinterPage.PageHeader.Font.Color = clWindowText
      PrinterPage.PageHeader.Font.Height = -11
      PrinterPage.PageHeader.Font.Name = 'Tahoma'
      PrinterPage.PageHeader.Font.Style = []
      PrinterPage.PageHeader.LeftTextAlignY = taTop
      PrinterPage.PageHeader.LeftTitle.Strings = (
        ''
        'Empresa :'
        'Vendedor :'
        'Atendente :'
        'Grupo :'
        'Categoria :'
        'Macro-Região :'
        'Micro-Região :'
        'Setor :'
        'Micro-Setor :'
        'Emitidos no Periodo de :'
        '')
      PrinterPage.PageHeader.RightTextAlignY = taTop
      PrinterPage.PageHeader.RightTitle.Strings = (
        '[Date Printed][Time Printed]')
      PrinterPage.PageSize.X = 210000
      PrinterPage.PageSize.Y = 297000
      PrinterPage.PaperSource = 15
      PrinterPage.ScaleFactor = 50
      PrinterPage._dxMeasurementUnits_ = 0
      PrinterPage._dxLastMU_ = 2
      ReportTitle.Font.Charset = ANSI_CHARSET
      ReportTitle.Font.Color = clWindowText
      ReportTitle.Font.Height = -15
      ReportTitle.Font.Name = 'Tahoma'
      ReportTitle.Font.Style = [fsBold]
      ReportTitle.Text = #13#10'Comercial - Analise de Pedidos Realizados no Periodo'#13#10#13#10
      BandColor = clWhite
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'Tahoma'
      BandFont.Style = [fsBold]
      EvenFont.Charset = ANSI_CHARSET
      EvenFont.Color = clWindowText
      EvenFont.Height = -11
      EvenFont.Name = 'Tahoma'
      EvenFont.Style = []
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      FooterFont.Charset = ANSI_CHARSET
      FooterFont.Color = clNavy
      FooterFont.Height = -13
      FooterFont.Name = 'Tahoma'
      FooterFont.Style = [fsBold]
      GridLineColor = clWhite
      GroupNodeFont.Charset = ANSI_CHARSET
      GroupNodeFont.Color = clNavy
      GroupNodeFont.Height = -13
      GroupNodeFont.Name = 'Tahoma'
      GroupNodeFont.Style = [fsBold]
      GroupNodeColor = clWhite
      HeaderFont.Charset = ANSI_CHARSET
      HeaderFont.Color = clNavy
      HeaderFont.Height = -13
      HeaderFont.Name = 'Tahoma'
      HeaderFont.Style = [fsBold]
      OddFont.Charset = ANSI_CHARSET
      OddFont.Color = clWindowText
      OddFont.Height = -11
      OddFont.Name = 'Tahoma'
      OddFont.Style = []
      Options = [tlpoHeaders, tlpoFooters, tlpoRowFooters, tlpoFlatCheckMarks]
      PreviewFont.Charset = ANSI_CHARSET
      PreviewFont.Color = clWindowText
      PreviewFont.Height = -11
      PreviewFont.Name = 'Tahoma'
      PreviewFont.Style = []
      RowFooterColor = clWhite
      RowFooterFont.Charset = ANSI_CHARSET
      RowFooterFont.Color = clRed
      RowFooterFont.Height = -13
      RowFooterFont.Name = 'Tahoma'
      RowFooterFont.Style = [fsBold]
    end
  end
  object PopUpMenu: TPopupMenu
    OwnerDraw = True
    Left = 216
    Top = 341
    object Exportarpara1: TMenuItem
      Caption = 'Exportar para...           '
      Hint = 'Exportar para...'
      object HTML: TMenuItem
        Caption = 'HTML'
        Hint = 'Exportar Relatorio no Formato HTML'
        OnClick = HTMLClick
      end
      object Excel: TMenuItem
        Caption = 'EXCEL'
        Hint = 'Exportar Relatorio no Formato EXCEL'
        OnClick = ExcelClick
      end
      object XML: TMenuItem
        Caption = 'XML && XSL'
        Hint = 'Exportar Relatorio no Formato XML '
        OnClick = XMLClick
      end
    end
    object Colunas1: TMenuItem
      Caption = 'Colunas...'
      Hint = 'Colunas...'
      OnClick = Colunas1Click
    end
  end
end
