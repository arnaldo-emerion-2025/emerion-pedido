...
type
  TfmManImpCtaNovoSemDesc = class(TForm)
    QuickRep1: TQuickRep;
    QRBand1: TQRBand;
    PedCt2: TwwQuery;
...
var
  fmManImpCtaNovoSemDesc: TfmManImpCtaNovoSemDesc;
...
function TfmManImpCtaNovoSemDesc.ShowModal(NumCta: integer; DteCta: TDateTime): integer;
begin
  //inherited Show;
  //
  PedCt2.Active := False;
...
function TfmManImpCtaNovoSemDesc.EnviaEmail(NumCta: integer): Boolean;
var
  strPDF: string;
...
function TfmManImpCtaNovoSemDesc.SavePDF(Path: string): boolean;
var
...
procedure TfmManImpCtaNovoSemDesc.qSubDetalheBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TfmManImpCtaNovoSemDesc.QuickRep1Preview(Sender: TObject);
begin
...
procedure TfmManImpCtaNovoSemDesc.PedCt2CalcFields(DataSet: TDataSet);
begin
  if PedCt2QTPRE2.AsFloat > 0 then
    PedCt2VALLIQTRB.AsFloat := fRound(PedCt2TOTITETRB.AsFloat / PedCt2QTPRE2.AsFloat, 2);
end;

procedure TfmManImpCtaNovoSemDesc.FormCreate(Sender: TObject);
begin
...
procedure TfmManImpCtaNovoSemDesc.QRDBText9Print(sender: TObject;
  var Value: string);
begin
  Value := PedCtaCODCLI.AsString + ' - ' + Trim(Value);
end;

procedure TfmManImpCtaNovoSemDesc.lb_MarcaPrint(sender: TObject;
  var Value: string);
begin
...
procedure TfmManImpCtaNovoSemDesc.edNomMRcPrint(sender: TObject;
  var Value: string);
begin
...
procedure TfmManImpCtaNovoSemDesc.QuickRep1BeforePrint(
...
procedure TfmManImpCtaNovoSemDesc.QRDBText40Print(sender: TObject;
  var Value: string);
begin
...
procedure TfmManImpCtaNovoSemDesc.QRDBText41Print(sender: TObject;
  var Value: string);
begin
  Value := 'I.E.: ' + Value;
end;

procedure TfmManImpCtaNovoSemDesc.PedCtaAfterOpen(DataSet: TDataSet);
begin
  if (Pos(PedCtaCGCEMP.AsString, '.') = 0) then
  begin
    if Length(Trim(PedCtaCGCEMP.AsString)) > 11 then
...
procedure TfmManImpCtaNovoSemDesc.QRDBText51Print(sender: TObject;
  var Value: string);
begin
  Value := fmManGdb.BuscaSimples('', 'FRT', '', 'select (CAST(ID as CHAR(1)) ||'' - ''|| DESCFRT) FRT FROM TIPFRT WHERE ID = ' + Value);
end;

procedure TfmManImpCtaNovoSemDesc.QRLabel17Print(sender: TObject;
  var Value: String);
begin
...
procedure TfmManImpCtaNovoSemDesc.QRDBText56Print(sender: TObject;
  var Value: String);
begin
  //Value := fomatfloat('#,##0.00', pedctaTotite.AsFloat+pedctaTotIPI.AsFloat+pedctaTotSUB.AsFloat);
end;

procedure TfmManImpCtaNovoSemDesc.bandSumarioBeforePrint(
