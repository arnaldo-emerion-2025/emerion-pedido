...
var
  verificador: string;
begin
  verificador := fmManGDB.BuscaSimples('PEDPAR', 'OBRLIBPED', '1=1');

  if verificador = '1' then
  begin
    if Trim(pedresOBSPRO.AsString) = '' then
    begin
      MessageBox(handle, 'Obrigatório informar observação do produto.', 'Pedido Programado', mb_Ok + MB_ICONWARNING);
      abort;
    end;
  end;
