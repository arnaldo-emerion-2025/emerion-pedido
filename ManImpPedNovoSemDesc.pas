...
type
  TfmManImpPedNovoSemDesc = class(TForm)
    PedRe2: TwwQuery;
...
var
  fmManImpPedNovoSemDesc: TfmManImpPedNovoSemDesc;

implementation

uses ManGDB, ufrmPreview, bbfuncao, bbgeral;

{$R *.DFM}

function TfmManImpPedNovoSemDesc.ShowModal(Numres: integer): integer;
begin
  //inherited Show;
  //
  Pedre2.Active := False;
  ...
  QuickRep1.PrevInitialZoom := qrZoom100;//qrZoomToWidth; //qrZoomOther;
  QuickRep1.Zoom := 100;
  //QuickRep1.Prepare;
...
function TfmManImpPedNovoSemDesc.EnviaEmail(Numres: integer): Boolean;
var
  strPDF: string;
...
function TfmManImpPedNovoSemDesc.SavePDF(Path: string): boolean;
var
...
procedure TfmManImpPedNovoSemDesc.qSubDetalheBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin

  if (trim(PedRe2OBSRE2.AsString) <> '')
    or (Trim(PedRe2NUMPEDCOMPRA.AsString) <> '')
    or (Trim(PedRe2NUMITEMCOMPRA.AsString) <> '')
    or ((Trim(Pedre2NOMMRC.AsString) <> '') and (IMPMRC = 'Sim')) then
  begin
  ...
  if ((Trim(Pedre2NOMMRC.AsString) <> '') and (IMPMRC = 'Sim')) then
  begin
  ...
  if (trim(PedRe2OBSRE2.AsString) <> '') then
    mmObsRe2.Enabled := True
  else
    mmObsRe2.Enabled := False;

  if (Trim(PedRe2NUMPEDCOMPRA.AsString) <> '') then
  begin
  ...
  if (Trim(PedRe2NUMITEMCOMPRA.AsString) <> '') then
  begin
...
procedure TfmManImpPedNovoSemDesc.PreviewPerson(Sender: TObject);
begin
  FrmPreview.showmodal;
end;

procedure TfmManImpPedNovoSemDesc.FormCreate(Sender: TObject);
begin
  ...
  QuickRep1.PreviewWidth := QuickRep1.Width;
  QuickRep1.PreviewHeight := QuickRep1.Height;

end;

procedure TfmManImpPedNovoSemDesc.QuickRep1Preview(Sender: TObject);
begin
...
procedure TfmManImpPedNovoSemDesc.PedRe2CalcFields(DataSet: TDataSet);
begin
  if PedRe2QTPRE2.AsFloat > 0 then
    PedRe2VALLIQTRB.AsFloat := fRound(PedRe2TOTITETRB.AsFloat / PedRe2QTPRE2.AsFloat, 2);
end;

procedure TfmManImpPedNovoSemDesc.QRDBText9Print(sender: TObject;
  var Value: string);
begin
  Value := PedResCODCLI.AsString + ' - ' + Trim(Value);
end;

procedure TfmManImpPedNovoSemDesc.lb_MarcaPrint(sender: TObject;
  var Value: string);
begin
  {lb_Marca.Enabled := trim(PedRE2NOMMRC.AsString) <> '';
  lb_Marca.Enabled := IMPMRC = 'Sim';}
end;

procedure TfmManImpPedNovoSemDesc.edNomMRcPrint(sender: TObject;
  var Value: string);
begin
  {edNomMRc.Enabled := trim(PedRE2NOMMRC.AsString) <> '';
  edNomMRc.Enabled := IMPMRC = 'Sim';}
end;

procedure TfmManImpPedNovoSemDesc.QuickRep1BeforePrint(
  Sender: TCustomQuickRep; var PrintReport: Boolean);
begin

  if (GGus_Id <> 1) and (GUsu_Id <> 1) and (GUsu_Id <> 999) and (GFlgGer <> 'Sim') and
  (fmMangdb.BuscaSimples('PEDPAR','IMPPEDCAT_CUSTO_MK',' 1 = 1') = 'S') then
  begin
...
procedure TfmManImpPedNovoSemDesc.QRDBText40Print(sender: TObject;
  var Value: string);
begin
  Value := 'CNPJ: ' + fFormatCgcCpf(Value);
end;

procedure TfmManImpPedNovoSemDesc.QRDBText41Print(sender: TObject;
  var Value: string);
begin
  Value := 'I.E.: ' + Value;
end;

procedure TfmManImpPedNovoSemDesc.QRDBText51Print(sender: TObject;
  var Value: string);
begin
  If (Trim(Value) = '') then Value := '999';
  Value := fmManGdb.BuscaSimples('', 'FRT', '', 'select (CAST(ID as CHAR(1)) ||'' - ''|| DESCFRT) FRT FROM TIPFRT WHERE ID = ' + Value);
end;

procedure TfmManImpPedNovoSemDesc.QRDBText15Print(sender: TObject;
  var Value: String);
begin
 Value := fFormatCgcCpf(Value);
end;

procedure TfmManImpPedNovoSemDesc.FormShow(Sender: TObject);
begin
  QuickRep1.PreviewWidth := Screen.Width - 2;
  QuickRep1.PreviewHeight := Screen.Height - 2;
  QuickRep1.PrevInitialZoom := qrZoom100;//qrZoomToWidth; //qrZoomOther;
