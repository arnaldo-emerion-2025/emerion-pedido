...
type
  ...
  TfmPedR20_Cta = class(TfmPadrao)
    ...
    PedR17DTECTA: TDateTimeField;
    PedR17ID_PEDCTA: TIntegerField;
    grR17: TdxDBGrid;
    grR17CODEMP: TdxDBGridMaskColumn;
    grR17DTEFAT: TdxDBGridDateColumn;
    grR17NROLIB: TdxDBGridMaskColumn;
    ...
    grR17CLIENTE: TdxDBGridMemoColumn;
    grR17VENDEDOR: TdxDBGridMemoColumn;
    grR17CODITE: TdxDBGridMaskColumn;
    grR17DTECTA: TdxDBGridDateColumn;
    grR17ID_PEDCTA: TdxDBGridMaskColumn;
...
procedure TfmPedR20_Cta.Save(ADefaultExt, AFilter, AFileName: string; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TfmPedR20_Cta.FormCreate(Sender: TObject);
begin
  inherited;

  sBase := ' Select cta.CodEmp, ' + _BR +
    ' cta.dtecta, ' + _BR +
    ' lib.DteFat, ' + _BR +
    ' cta.numcta Id_Pedcta, ' + _BR +
    ' lib.SEQLIB NroLib, ' + _BR +
    ' coalesce(ct2.desct2,0.0) as DscPro, ' + _BR +
    ' coalesce(ct2.qtpct2,0.0) as qtpct2, ' + _BR +
    ' coalesce(re2.qtpre2,0.0) as qtpre2, ' + _BR +
    ' coalesce(lb2.ultqts,0.0) as qtplb2, ' + _BR +
    ' (coalesce(lb2.SldLb2,0.0) + coalesce(ct2.QtnCt2,0.0)) as SldLb2 , ' + _BR +
    ' lb2.UltQts, ' + _BR +
    ' Round(ct2.vlqct2,4) as VlqLb2, ' + _BR +
    ' Round(((coalesce(lb2.SldLb2,0.0) + coalesce(ct2.QtnCt2,0.0)) * (ct2.vlqct2)),4) as TotLb2, ' + _BR +
    ' RTrim(LTrim(cli.ApeCli)) as Cliente, ' + _BR +
    ' RTrim(LTrim(ven.ApeVen)) as Vendedor, ' + _BR +
  ...
  ' join estpro pro on pro.codclp = ct2.codclp ' + _BR +
    ...
    ' and res.sitres <> ' + QuotedStr('Cancelado');

  SaveDialog.InitialDir := ExtractFilePath(Application.ExeName);

end;

procedure TfmPedR20_Cta.bSelecionarClick(Sender: TObject);
var
  strAux: string;
begin
  sFiltro := '';
  ...
  sOrdem := ' Order by pedct2.numcta,pedct2.CodClp,pedct2.CodGru,pedct2.CodSub,pedct2.CodPro';
  ...
  if Trim(fLimpaStr(EdDteRes1.Text)) <> '' then
    sFiltro := sFiltro + ' and cta.dtecta >= ' + QuotedStr(fDateToSQL(EdDteRes1.Date)) + _BR;
  if Trim(fLimpaStr(EdDteRes2.Text)) <> '' then
    sFiltro := sFiltro + ' and cta.dtecta <= ' + QuotedStr(fDateToSQL(EdDteRes2.Date)) + _BR;

  if Trim(EdCodGve.Text) <> '' then
    sFiltro := sFiltro + ' and Ven.CodGve = ' + QuotedStr(EdCodGve.Text) + _BR;
  if Trim(EdCodVen.Text) <> '' then
    sFiltro := sFiltro + ' and cta.CodVen = ' + QuotedStr(EdCodVen.Text) + _BR;
  if Trim(EdCodCli.Text) <> '' then
    sFiltro := sFiltro + ' and cta.CodCli = ' + QuotedStr(EdCodCli.Text) + _BR;
  if Trim(EdCodEmp.Text) <> '' then
    sFiltro := sFiltro + ' and cta.CodEmp = ' + QuotedStr(EdCodEmp.Text) + _BR;
  if Trim(EdCodClp.Text) <> '' then
    sFiltro := sFiltro + ' and ct2.CodClp = ' + QuotedStr(EdCodClp.Text) + _BR;
  if Trim(EdCodGru.Text) <> '' then
    sFiltro := sFiltro + ' and ct2.CodGru = ' + QuotedStr(EdCodGru.Text) + _BR;
  if Trim(EdCodSub.Text) <> '' then
    sFiltro := sFiltro + ' and ct2.CodSub = ' + QuotedStr(EdCodSub.Text) + _BR;
  if Trim(EdCodPro.Text) <> '' then
    sFiltro := sFiltro + ' and ct2.CodPro = ' + QuotedStr(EdCodPro.Text) + _BR;
  if Trim(EdCodCat.Text) <> '' then
    sFiltro := sFiltro + ' and pro.CodCat = ' + QuotedStr(EdCodCat.Text) + _BR;
  if Trim(EdCodTip.Text) <> '' then
    sFiltro := sFiltro + ' and pro.CodTip = ' + QuotedStr(EdCodTip.Text) + _BR;
  if Trim(EdCodMrc.Text) <> '' then
    sFiltro := sFiltro + ' and pro.CodMrc = ' + QuotedStr(EdCodMrc.Text) + _BR;

  {if GFlgAce = 'Sim' then
    sFiltro := sFiltro + ' and PedLib.CodTip = 1 and not PedLib.FlgDif = ' + QuotedStr('Sim');}

  sFiltro := sFiltro + ' and (coalesce(Lb2.SldLb2,0) + coalesce(Ct2.QtnCt2,0)) > ' + QuotedStr('0') + _BR;
...
procedure TfmPedR20_Cta.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if key = 116 then
...
procedure TfmPedR20_Cta.FormShow(Sender: TObject);
begin
  inherited;
  EdCodEmp.SetFocus;
...
procedure TfmPedR20_Cta.bImprimirClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR20_Cta.bPsqEmpClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR20_Cta.EdCodEmpExit(Sender: TObject);
begin
  if Trim(EdCodEmp.Text) <> '' then
  begin
...
procedure TfmPedR20_Cta.bPsqVenClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR20_Cta.EdCodVenExit(Sender: TObject);
begin
  inherited;
  if Trim(EdCodVen.Text) <> '' then
  begin
...
procedure TfmPedR20_Cta.bPsqCliClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR20_Cta.EdCodCliExit(Sender: TObject);
begin
  inherited;
  if Trim(EdCodCli.Text) <> '' then
  begin
...
procedure TfmPedR20_Cta.EdCodCatExit(Sender: TObject);
begin
  inherited;
  if Trim(EdCodCat.Text) <> '' then
  begin
...
procedure TfmPedR20_Cta.bPsqCatClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR20_Cta.EdCodTipExit(Sender: TObject);
begin
  inherited;
  if Trim(EdCodTip.Text) <> '' then
  begin
...
procedure TfmPedR20_Cta.bPsqTipClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR20_Cta.EdCodMrcExit(Sender: TObject);
begin
  inherited;
  if Trim(EdCodMrc.Text) <> '' then
  begin
...
procedure TfmPedR20_Cta.bPsqMrcClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR20_Cta.EdCodClpExit(Sender: TObject);
begin
  inherited;
  if Trim(EdCodClp.Text) <> '' then
  begin
...
procedure TfmPedR20_Cta.bPsqClpClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR20_Cta.EdCodGruExit(Sender: TObject);
begin
  inherited;
  if Trim(EdCodGru.Text) <> '' then
  begin
...
procedure TfmPedR20_Cta.bPsqGruClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR20_Cta.EdCodSubExit(Sender: TObject);
begin
  inherited;
  if Trim(EdCodSub.Text) <> '' then
  begin
...
procedure TfmPedR20_Cta.bPsqSubClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR20_Cta.EdCodProExit(Sender: TObject);
begin
  inherited;
  if Trim(EdCodPro.Text) <> '' then
  begin
...
procedure TfmPedR20_Cta.bPsqProClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR20_Cta.EdCodGveExit(Sender: TObject);
begin
  inherited;
  if Trim(EdCodGve.Text) <> '' then
  begin
...
procedure TfmPedR20_Cta.bPsqGveClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR20_Cta.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  Action := CaFree;
end;

procedure TfmPedR20_Cta.FormDestroy(Sender: TObject);
begin
  inherited;
  fmPedR20_Cta := nil;
end;

procedure TfmPedR20_Cta.Colunas1Click(Sender: TObject);
begin
  inherited;
  ...
  try
    ...
    fmManCol.Synchronize(fmPedR20_Cta.grR17);
