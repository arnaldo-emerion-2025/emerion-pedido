inherited fmPedR03: TfmPedR03
  Left = 0
  Caption = 'Relação simplificada de clientes'
  ClientHeight = 482
  ClientWidth = 786
  Position = poDesigned
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PaintBox: TPaintBox
    Left = 0
    Top = 0
    Width = 786
    Height = 482
    Align = alClient
    OnPaint = PaintBoxPaint
  end
  object Label6: TLabel
    Left = 4
    Top = 6
    Width = 50
    Height = 13
    Caption = 'Vendedor:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label7: TLabel
    Left = 4
    Top = 29
    Width = 55
    Height = 13
    Caption = 'Atendente:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label3: TLabel
    Left = 4
    Top = 52
    Width = 79
    Height = 13
    Caption = 'Transportadora:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object bPsqVen: TSpeedButton
    Left = 165
    Top = 3
    Width = 23
    Height = 20
    Hint = 'Pesquisar Vendedores Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    ParentShowHint = False
    ShowHint = True
    OnClick = bPsqVenClick
  end
  object bPsqAtd: TSpeedButton
    Left = 165
    Top = 26
    Width = 23
    Height = 20
    Hint = 'Pesquisar Atendentes Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    ParentShowHint = False
    ShowHint = True
    OnClick = bPsqAtdClick
  end
  object bPsqTra: TSpeedButton
    Left = 165
    Top = 50
    Width = 23
    Height = 20
    Hint = 'Pesquisar Transportadoras Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    ParentShowHint = False
    ShowHint = True
    OnClick = bPsqTraClick
  end
  object Label5: TLabel
    Left = 4
    Top = 75
    Width = 33
    Height = 13
    Caption = 'Grupo:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object bPsqGCl: TSpeedButton
    Left = 165
    Top = 73
    Width = 23
    Height = 20
    Hint = 'Pesquisar Grupo de Clientes Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    ParentShowHint = False
    ShowHint = True
    OnClick = bPsqGClClick
  end
  object Label8: TLabel
    Left = 4
    Top = 98
    Width = 51
    Height = 13
    Caption = 'Categoria:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object bPsqCCl: TSpeedButton
    Left = 165
    Top = 96
    Width = 23
    Height = 20
    Hint = 'Pesquisar Categorias de Clientes Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    ParentShowHint = False
    ShowHint = True
    OnClick = bPsqCClClick
  end
  object Label11: TLabel
    Left = 4
    Top = 121
    Width = 67
    Height = 13
    Caption = 'Macro-região:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object bPsqMcr: TSpeedButton
    Left = 165
    Top = 119
    Width = 23
    Height = 20
    Hint = 'Pesquisar Macro Regiões Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    ParentShowHint = False
    ShowHint = True
    OnClick = bPsqMcrClick
  end
  object Label12: TLabel
    Left = 4
    Top = 144
    Width = 63
    Height = 13
    Caption = 'Micro-região:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object bPsqMrg: TSpeedButton
    Left = 165
    Top = 142
    Width = 23
    Height = 20
    Hint = 'Pesquisar Micro Regiões Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    ParentShowHint = False
    ShowHint = True
    OnClick = bPsqMrgClick
  end
  object Label13: TLabel
    Left = 4
    Top = 167
    Width = 30
    Height = 13
    Caption = 'Setor:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object bPsqSet: TSpeedButton
    Left = 165
    Top = 165
    Width = 23
    Height = 20
    Hint = 'Pesquisar Setores Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    ParentShowHint = False
    ShowHint = True
    OnClick = bPsqSetClick
  end
  object Label14: TLabel
    Left = 4
    Top = 189
    Width = 58
    Height = 13
    Caption = 'Micro-setor:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object bPsqMst: TSpeedButton
    Left = 165
    Top = 187
    Width = 23
    Height = 20
    Hint = 'Pesquisar Micro Setores Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    ParentShowHint = False
    ShowHint = True
    OnClick = bPsqMstClick
  end
  object bSelecionar: TBitBtn
    Left = 483
    Top = 164
    Width = 150
    Height = 43
    Caption = '&Selecionar'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 19
    OnClick = bSelecionarClick
    Glyph.Data = {
      
      3232323232323232323232323232323232323232323232323232}
  end
  object grCli: TdxDBGrid
    Left = 2
    Top = 209
    Width = 782
    Height = 273
    Bands = <
      item
        Width = 1224
      end>
    DefaultLayout = False
    HeaderPanelRowCount = 1
    KeyField = 'CODCLI'
    ShowGroupPanel = True
    SummaryGroups = <>
    SummarySeparator = ', '
    BorderStyle = bsNone
    Color = clInfoBk
    Font.Charset = ANSI_CHARSET
    Font.Color = clNavy
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    PopupMenu = PopUpMenu
    TabOrder = 21
    BandColor = clMenu
    BandFont.Charset = ANSI_CHARSET
    BandFont.Color = clWindowText
    BandFont.Height = -11
    BandFont.Name = 'Tahoma'
    BandFont.Style = [fsBold]
    DataSource = DsCli
    Filter.Active = True
    Filter.AutoDataSetFilter = True
    Filter.Criteria = {00000000}
    GroupPanelColor = clAppWorkSpace
    GroupPanelFontColor = clInfoBk
    GroupNodeColor = clInfoBk
    HeaderFont.Charset = ANSI_CHARSET
    HeaderFont.Color = clWindowText
    HeaderFont.Height = -11
    HeaderFont.Name = 'Tahoma'
    HeaderFont.Style = [fsBold]
    LookAndFeel = lfFlat
    OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEnterShowEditor, edgoImmediateEditor, edgoTabThrough, edgoVertThrough]
    OptionsCustomize = [edgoBandSizing, edgoColumnMoving, edgoColumnSizing, edgoFullSizing]
    OptionsDB = [edgoCanNavigation, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoPreview, edgoRowSelect, edgoUseBitmap]
    PreviewFont.Charset = ANSI_CHARSET
    PreviewFont.Color = clBlue
    PreviewFont.Height = -11
    PreviewFont.Name = 'Tahoma'
    PreviewFont.Style = []
    PreviewLines = 0
    RowFooterColor = clInfoBk
    RowFooterTextColor = clRed
    ShowGrid = False
    ShowHiddenInCustomizeBox = True
    ShowRowFooter = True
    OnReloadGroupList = grCliReloadGroupList
    object grCliCODCLI: TdxDBGridMaskColumn
      Width = 67
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CODCLI'
    end
    object grCliNOMCLI: TdxDBGridMaskColumn
      Width = 270
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NOMCLI'
    end
    object grCliAPECLI: TdxDBGridMaskColumn
      Width = 160
      BandIndex = 0
      RowIndex = 0
      FieldName = 'APECLI'
    end
    object grCliCGCCPF: TdxDBGridMaskColumn
      Width = 104
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CGCCPF'
    end
    object grCliENDERECO: TdxDBGridMaskColumn
      Width = 430
      BandIndex = 0
      RowIndex = 0
      FieldName = 'ENDERECO'
    end
    object grCliCEPCLI: TdxDBGridColumn
      Caption = 'Cep'
      Width = 83
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CEPCLI'
    end
    object grCliTELEFONE: TdxDBGridMaskColumn
      Width = 110
      BandIndex = 0
      RowIndex = 0
      FieldName = 'TELEFONE'
    end
  end
  object bImprimir: TBitBtn
    Left = 635
    Top = 164
    Width = 150
    Height = 43
    Caption = '&Imprimir'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 20
    OnClick = bImprimirClick
    Glyph.Data = {
      
      D000}
  end
  object EdPsqCodTra: TdxColorEdit
    Left = 85
    Top = 48
    Width = 79
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 4
    OnExit = EdPsqCodTraExit
    OnKeyPress = EdPsqCodVenKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdPsqCodAtd: TdxColorEdit
    Left = 85
    Top = 25
    Width = 79
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 2
    OnExit = EdPsqCodAtdExit
    OnKeyPress = EdPsqCodVenKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdPsqCodVen: TdxColorEdit
    Left = 85
    Top = 2
    Width = 79
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 0
    OnExit = EdPsqCodVenExit
    OnKeyPress = EdPsqCodVenKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdPsqNomTra: TdxColorEdit
    Left = 189
    Top = 48
    Width = 444
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 5
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clBlack
    StoredValues = 2
  end
  object EdPsqNomAtd: TdxColorEdit
    Left = 189
    Top = 25
    Width = 444
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 3
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clBlack
    StoredValues = 2
  end
  object EdPsqNomVen: TdxColorEdit
    Left = 189
    Top = 2
    Width = 444
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 1
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clBlack
    StoredValues = 2
  end
  object EdPsqCodGCl: TdxColorEdit
    Left = 85
    Top = 71
    Width = 79
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 6
    OnExit = EdPsqCodGClExit
    OnKeyPress = EdPsqCodVenKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdPsqNomGCl: TdxColorEdit
    Left = 189
    Top = 71
    Width = 444
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 7
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clBlack
    StoredValues = 2
  end
  object EdPsqCodCCl: TdxColorEdit
    Left = 85
    Top = 94
    Width = 79
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 8
    OnExit = EdPsqCodCClExit
    OnKeyPress = EdPsqCodVenKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdPsqNomCCl: TdxColorEdit
    Left = 189
    Top = 94
    Width = 444
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 9
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clBlack
    StoredValues = 2
  end
  object EdPsqCodMcr: TdxColorEdit
    Left = 85
    Top = 117
    Width = 79
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 10
    OnExit = EdPsqCodMcrExit
    OnKeyPress = EdPsqCodVenKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdPsqNomMcr: TdxColorEdit
    Left = 189
    Top = 117
    Width = 444
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 11
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clBlack
    StoredValues = 2
  end
  object EdPsqCodMrg: TdxColorEdit
    Left = 85
    Top = 140
    Width = 79
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 12
    OnExit = EdPsqCodMrgExit
    OnKeyPress = EdPsqCodVenKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdPsqNomMrg: TdxColorEdit
    Left = 189
    Top = 140
    Width = 444
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 13
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clBlack
    StoredValues = 2
  end
  object EdPsqCodSet: TdxColorEdit
    Left = 85
    Top = 163
    Width = 79
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 14
    OnExit = EdPsqCodSetExit
    OnKeyPress = EdPsqCodVenKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdPsqNomSet: TdxColorEdit
    Left = 189
    Top = 163
    Width = 292
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 15
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clBlack
    StoredValues = 2
  end
  object EdPsqCodMst: TdxColorEdit
    Left = 85
    Top = 185
    Width = 79
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 16
    OnExit = EdPsqCodMstExit
    OnKeyPress = EdPsqCodVenKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdPsqNomMst: TdxColorEdit
    Left = 189
    Top = 185
    Width = 292
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 17
    CharCase = ecUpperCase
    MaxLength = 10
    CorDeFoco = clBlack
    StoredValues = 2
  end
  object rgEndereco: TRadioGroup
    Left = 634
    Top = -2
    Width = 150
    Height = 163
    Caption = ' Endereço '
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ItemIndex = 3
    Items.Strings = (
      'Entrega'
      'Compras'
      'Cobrança'
      'Faturamento')
    ParentFont = False
    TabOrder = 18
  end
  object FinCli: TwwQuery
    CachedUpdates = True
    DatabaseName = 'ISade'
    SQL.Strings = (
      'Select CodCli,NomCli,ApeCli,CefCli as CepCli,'
      '           FormatCgcCpf(CgcCli) as CgcCpf,'
      
        '           Endereco(TefCli,EnfCli,NrfCli,RffCli,BafCli,CifCli,Uf' +
        'fCli) as Endereco,'
      
        '           TextoOcor('#39'('#39',Pt1Cli,'#39')'#39','#39' '#39',Fo1Cli,'#39#39','#39#39') as Telefon' +
        'e'
      'From FinCli'
      'Where CodCli  = :CodCli'
      'Order by CodCli')
    ValidateWithMask = True
    Left = 3
    Top = 381
    ParamData = <
      item
        DataType = ftInteger
        Name = 'CodCli'
        ParamType = ptInput
      end>
    object FinCliCODCLI: TIntegerField
      Alignment = taLeftJustify
      DisplayLabel = 'Código'
      FieldName = 'CODCLI'
    end
    object FinCliNOMCLI: TStringField
      DisplayLabel = 'Nome/Razão Social'
      FieldName = 'NOMCLI'
      FixedChar = True
      Size = 70
    end
    object FinCliCGCCPF: TStringField
      DisplayLabel = 'CNPJ/CPF'
      FieldName = 'CGCCPF'
      FixedChar = True
      Size = 18
    end
    object FinCliENDERECO: TStringField
      DisplayLabel = 'Endereço'
      FieldName = 'ENDERECO'
      FixedChar = True
      Size = 172
    end
    object FinCliTELEFONE: TStringField
      DisplayLabel = 'Telefone'
      FieldName = 'TELEFONE'
      FixedChar = True
      Size = 100
    end
    object FinCliAPECLI: TStringField
      DisplayLabel = 'Apelido'
      FieldName = 'APECLI'
      FixedChar = True
    end
    object FinCliCEPCLI: TStringField
      FieldName = 'CEPCLI'
      EditMask = '!99999\-999;0; '
      FixedChar = True
      Size = 8
    end
  end
  object DsCli: TwwDataSource
    DataSet = FinCli
    Left = 31
    Top = 381
  end
  object quSql: TwwQuery
    DatabaseName = 'ISade'
    UniDirectional = True
    ValidateWithMask = True
    Left = 3
    Top = 409
  end
  object SaveDialog: TSaveDialog
    FileName = 'ExpGrid'
    Options = [ofOverwritePrompt, ofHideReadOnly]
    Left = 31
    Top = 409
  end
  object dxComponentPrinter: TdxComponentPrinter
    CurrentLink = dxComponentPrinterLink1
    DateFormat = 1
    PreviewOptions.Caption = 'Visualizando Relatorio'
    PreviewOptions.RegistryPath = 'Software\Developer Express\PrintingSystem\DesignTime'
    TimeFormat = 1
    Left = 31
    Top = 437
    object dxComponentPrinterLink1: TdxDBGridReportLink
      Active = True
      Component = grCli
      DateTime = 40210.4065592014
      DesignerHelpContext = 0
      PrinterPage.DMPaper = 9
      PrinterPage.Footer = 6350
      PrinterPage.Header = 6350
      PrinterPage.Margins.Bottom = 12700
      PrinterPage.Margins.Left = 12700
      PrinterPage.Margins.Right = 12700
      PrinterPage.Margins.Top = 58500
      PrinterPage.Orientation = poLandscape
      PrinterPage.PageFooter.Font.Charset = DEFAULT_CHARSET
      PrinterPage.PageFooter.Font.Color = clWindowText
      PrinterPage.PageFooter.Font.Height = -11
      PrinterPage.PageFooter.Font.Name = 'Tahoma'
      PrinterPage.PageFooter.Font.Style = []
      PrinterPage.PageFooter.RightTitle.Strings = (
        'Pagina No. [Page #]')
      PrinterPage.PageHeader.Background.Brush.Color = clBlack
      PrinterPage.PageHeader.Background.PictureMode = ppmTile
      PrinterPage.PageHeader.CenterTextAlignY = taTop
      PrinterPage.PageHeader.Font.Charset = DEFAULT_CHARSET
      PrinterPage.PageHeader.Font.Color = clWindowText
      PrinterPage.PageHeader.Font.Height = -11
      PrinterPage.PageHeader.Font.Name = 'Tahoma'
      PrinterPage.PageHeader.Font.Style = []
      PrinterPage.PageHeader.LeftTextAlignY = taTop
      PrinterPage.PageHeader.LeftTitle.Strings = (
        ''
        'Vendedor :'
        'Atendente :'
        'Transportadora :'
        'Grupo :'
        'Categoria :'
        'Macro-Região :'
        'Micro-Região :'
        'Setor :'
        'Micro-Setor :'
        '')
      PrinterPage.PageHeader.RightTextAlignY = taTop
      PrinterPage.PageHeader.RightTitle.Strings = (
        '[Date Printed][Time Printed]')
      PrinterPage.PageSize.X = 210000
      PrinterPage.PageSize.Y = 297000
      PrinterPage.PaperSource = 15
      PrinterPage.ScaleFactor = 83
      PrinterPage._dxMeasurementUnits_ = 0
      PrinterPage._dxLastMU_ = 2
      ReportTitle.Font.Charset = ANSI_CHARSET
      ReportTitle.Font.Color = clWindowText
      ReportTitle.Font.Height = -15
      ReportTitle.Font.Name = 'Tahoma'
      ReportTitle.Font.Style = [fsBold]
      ReportTitle.Text = #13#10'Relação simplificada de clientes'#13#10#13#10
      BandColor = clWhite
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'Tahoma'
      BandFont.Style = [fsBold]
      EvenFont.Charset = DEFAULT_CHARSET
      EvenFont.Color = clWindowText
      EvenFont.Height = -11
      EvenFont.Name = 'Times New Roman'
      EvenFont.Style = []
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Times New Roman'
      Font.Style = []
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clNavy
      FooterFont.Height = -13
      FooterFont.Name = 'Times New Roman'
      FooterFont.Style = [fsBold]
      GridLineColor = clWhite
      GroupNodeFont.Charset = DEFAULT_CHARSET
      GroupNodeFont.Color = clNavy
      GroupNodeFont.Height = -13
      GroupNodeFont.Name = 'Times New Roman'
      GroupNodeFont.Style = [fsBold]
      GroupNodeColor = clWhite
      HeaderFont.Charset = ANSI_CHARSET
      HeaderFont.Color = clNavy
      HeaderFont.Height = -13
      HeaderFont.Name = 'Times New Roman'
      HeaderFont.Style = [fsBold]
      OddFont.Charset = DEFAULT_CHARSET
      OddFont.Color = clWindowText
      OddFont.Height = -11
      OddFont.Name = 'Times New Roman'
      OddFont.Style = []
      Options = [tlpoHeaders, tlpoFooters, tlpoRowFooters, tlpoFlatCheckMarks]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clWindowText
      PreviewFont.Height = -11
      PreviewFont.Name = 'Times New Roman'
      PreviewFont.Style = []
      RowFooterColor = clWhite
      RowFooterFont.Charset = DEFAULT_CHARSET
      RowFooterFont.Color = clRed
      RowFooterFont.Height = -13
      RowFooterFont.Name = 'Times New Roman'
      RowFooterFont.Style = [fsBold]
    end
  end
  object PopUpMenu: TPopupMenu
    OwnerDraw = True
    Left = 3
    Top = 437
    object Exportarpara1: TMenuItem
      Caption = 'Exportar para...           '
      Hint = 'Exportar para...'
      object HTML: TMenuItem
        Caption = 'HTML'
        Hint = 'Exportar Relatorio no Formato HTML'
        OnClick = HTMLClick
      end
      object Excel: TMenuItem
        Caption = 'EXCEL'
        Hint = 'Exportar Relatorio no Formato EXCEL'
        OnClick = ExcelClick
      end
      object XML: TMenuItem
        Caption = 'XML && XSL'
        Hint = 'Exportar Relatorio no Formato XML '
        OnClick = XMLClick
      end
    end
  end
end
