...
begin
  ...
  sFiltro := ' Where not PedRes.SitRes = ' + QuotedStr('Nao Concluido') + ' and not PedRes.SitRes = ' + QuotedStr('Faturado') + ' and not PedRes.SitRes = ' +
    QuotedStr('Faturado com Saldo nao Atendido');

  if Trim(EdCodEmp.Text) <> '' then
    sFiltro := sFiltro + ' and PedRes.CodEmp = ' + QuotedStr(EdCodEmp.Text);
  if Trim(EdCodVen.Text) <> '' then
    sFiltro := sFiltro + ' and PedRes.CodVen = ' + QuotedStr(EdCodVen.Text);
  if Trim(EdCodCli.Text) <> '' then
    sFiltro := sFiltro + ' and PedRes.CodCli = ' + QuotedStr(EdCodCli.Text);
  if Trim(EdCodEmp.Text) <> '' then
    sFiltro := sFiltro + ' and PedRe2.CodEit = ' + QuotedStr(EdCodEmp.Text);
  if Trim(EdCodClp.Text) <> '' then
    sFiltro := sFiltro + ' and PedRe2.CodClp = ' + QuotedStr(EdCodClp.Text);
  if Trim(EdCodGru.Text) <> '' then
    sFiltro := sFiltro + ' and PedRe2.CodGru = ' + QuotedStr(EdCodGru.Text);
  if Trim(EdCodSub.Text) <> '' then
    sFiltro := sFiltro + ' and PedRe2.CodSub = ' + QuotedStr(EdCodSub.Text);
  if Trim(EdCodPro.Text) <> '' then
    sFiltro := sFiltro + ' and PedRe2.CodPro = ' + QuotedStr(EdCodPro.Text);

  if EdPsqDteRes1.Date > 0 then
    sFiltro := sFiltro + ' and PedRes.DteRes >= ' + QuotedStr(fDateToSQL(EdPsqDteRes1.Date));
  if EdPsqDteRes2.Date > 0 then
  ...
  begin
    ...
    Text := ' Select PedR08.*,' +
      '        EstPro.DscPro,' +
      '        PedR08.CodClp || _UNICODE_FSS ' + QuotedStr('-') + ' || PedR08.CodGru || _UNICODE_FSS ' + QuotedStr('.') + ' || PedR08.CodSub || _UNICODE_FSS '
        + QuotedStr('.') + ' || PedR08.CodPro as CodIte' +
